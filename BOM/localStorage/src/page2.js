...
function get_message_list() {
  const list = window.localStorage.getItem('_message_1')
  if (!list) return
...
button.addEventListener('click', () => {
  ...
  const message_list = window.localStorage.getItem('_message_2')
  if(!message_list) {
    const list = [{
      id: Date.now(),
      text: input.value.trim()
    }]
    window.localStorage.setItem('_message_2', JSON.stringify(list))
  } else {
    try {
